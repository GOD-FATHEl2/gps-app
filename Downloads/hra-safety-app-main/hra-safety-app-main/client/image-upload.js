...
class ImageUploadManager {
    ...
    async uploadImageToServer(imageData) {
        ...
        try {
            ...
            let authToken = '';
            if (window.getFreshToken) {
                try {
                    authToken = await window.getFreshToken();
                } catch (error) {
                    console.warn('⚠️ Could not get fresh token, using fallback:', error);
                    authToken = window.token || localStorage.getItem('token') || localStorage.getItem('hra_token') || '';
                }
            } else {
                authToken = window.token || localStorage.getItem('token') || localStorage.getItem('hra_token') || '';
            }
            ...
            if (!authToken) {
                throw new Error('Ingen giltig token - logga in igen');
            }
            ...
            if (error.message.includes('logga in igen') || error.message.includes('Autentisering')) {
